...
// This class was created to provide a dynamic array implementation that is able to return a regular array easily
...
public class Vector <T> {
  ...
  private static <T> T[] createArray(int size, T defaultValue) {
    // Terrible casting needed because of runtime Objects and rules against generic arrays in java
  ...
  public void add (T data) {
    if (usedSize+1 < size) {
      ...
      storage = createArray(size, defaultVal);
      int i = 0;
      // "Copies" the elements (it really uses references) hopefully doesn't create memory leaks by keeping references to old allocations
